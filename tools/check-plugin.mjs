...
const BUS_CASES = [
  ...
  { name: "BUS-09 thêm thủ công đếm vào sức chứa nhưng không tự chốt xe", run: (m) => { m.autoAssign(101, 1, true); m.assign(701, 1, { role: "super" }); const fullStillOpen = m.buses[0].status === "BOARDING" && m.members.filter((x) => x.busId === 1).length === 2; const r = m.autoAssign(102, 1, true); return fullStillOpen && m.buses[0].status === "CLOSED" && r.busId === 2; } },
