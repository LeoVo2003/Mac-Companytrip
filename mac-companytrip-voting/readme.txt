...
= 1.10.50 =
- Phân xe: gán BTC/Hoa tiêu vào xe giờ kèm trọn nhóm người thân đi cùng (trước đây chỉ thêm mình người BTC); chuyển/xóa BTC cũng kéo theo đúng nhóm. Thêm test hồi quy BUS-25.
...
= 1.10.42 =
- BTC/Hoa tiêu chấm được trở lại: handler thêm/sửa/xóa hạng mục thi đua + chấm điểm và handler xếp hạng trò chơi lớn trước đó nằm trong khối bind riêng của super admin nên nút hiện mà không hoạt động; giờ bind cho mức operator (server vẫn chặn đúng mức operator).

= 1.10.41 =
- Super Admin xuất được "Xuất tổng 5 xe" mọi thời điểm (bỏ chốt chặn phải đóng đủ 5 xe ở cả nút lẫn server); xuất từng xe giữ nguyên quyền super/HDV-xe-mình/BTC.
...
= 1.10.38 =
- Mail QR: ảnh QR nhúng CID là đường ảnh duy nhất (không còn nhánh mso/!mso) — new Outlook/Gmail/mobile render ảnh trong thân mail, không bị chặn "người gửi chưa tin cậy" như khi tải từ URL ngoài; link mở ảnh công khai giữ làm dự phòng.
- Bỏ emoji trong nút đỏ của mail (new Outlook hiển thị lỗi glyph).

= 1.10.37 =
- Nút "Bật cổng chấm điểm" về đúng chuẩn hành động chính: nền gradient đỏ–cam liền khối khi nghỉ, hover sáng lên + đổ bóng (trước đó nghỉ trắng, hover mới hóa gradient).

= 1.10.36 =
- Select trong bảng trơn (đổi xe HDV, gán xe bảng TỰ TÚC / chưa phân xe) theo đúng chuẩn select hệ thống: cao 44px, bo 12px, mũi tên riêng, focus ring đỏ.
- Ô Đi kèm bảng TỰ TÚC làm lại: chip "+N đi kèm" bo tròn, bấm xổ dropdown nền trắng đổ bóng liệt kê tên từng người; không còn chữ trần dính details cũ.

= 1.10.35 =
- Nút "Tự túc / Bỏ tự túc" từng người trong bảng Nhân sự: đánh dấu phát sinh đi xe riêng không cần import lại; người được đánh dấu nằm bảng TỰ TÚC chờ gán tay.
- Bảng TỰ TÚC: cột Đi kèm hiện +N kèm xổ xem tên từng người đi kèm; người đi kèm thừa hưởng cờ tự túc của người chính khi import.
- Sửa lệch cột Thao tác bảng xe (nút cùng trục trái tiêu đề); tag trạng thái không rớt dòng trên tablet; form thêm tài khoản HDV xếp 2 hàng trên tablet.

= 1.10.34 =
- Bảng nhân sự: ghi chú "Đi kèm theo QR người chính" tăng line-height 3.4 để không bị cắt dòng trong ô thao tác.

= 1.10.33 =
- Nút "Xóa toàn bộ nhân sự" trong tab Nhân sự & QR: chỉ Super Admin, phải gõ XOA để xác nhận; xóa sạch nhân sự kèm phiếu, check-in, thành viên xe, miễn trừ, quyền vote lại (audit PEOPLE_WIPED).
- Sửa lệch hàng bảng nhân sự: ghi chú "Đi kèm theo QR người chính" cùng trục giữa với các nút trong ô thao tác.
...
- Chốt xe theo sức chứa giờ chỉ chạy trong luồng quét QR Trạm 1 (xe đầy tự chốt + mở xe kế ngay lượt quét đó).
...
= 1.10.27 =
- Người tự túc (Đi xe = Không) mà vẫn quét QR Trạm 1 = đổi ý đi xe: tự động nhận đúng người quét lên xe đang mở (1 chỗ), không kéo theo người nhà tự túc không quét; người nhà còn lại vẫn chờ trong bảng TỰ TÚC để gán tay.
...
- Dashboard phân biệt rõ lỗi không nhận JSON (file vượt upload_max_filesize/post_max_size hoặc hết phiên) với lỗi JSON hỏng.

= 1.10.21 =
- Sửa guard ZipArchive đụng luật PHP 7.4 của check-plugin (bản 1.10.20 chưa qua check); nội dung reader XLSX giữ nguyên.
...
- Nhóm người thân đọc 2 pass: người chính nằm giữa nhóm (hoặc người đi kèm đứng trước) vẫn ghép đúng; báo lỗi rõ nhóm không có / có nhiều người chính.
- Email không còn bắt buộc với người chính (vẫn có QR + check-in); dashboard vẫn cảnh báo danh sách thiếu email để bổ sung sau.
...
- Super Admin vẫn đóng xe sớm / mở xe khác / mở lại xe đã chốt thủ công ngay trong bảng phân xe khi cần; mở xe khác trả xe đang nhận về hàng chờ.
- check-plugin: 16 test case Phân xe (BUS-01…BUS-16) đồng bộ luật sức chứa + tự chốt/mở.

= 1.10.14 =
- ket-qua-tong: bấm "Công bố hạng nhì" là á quân và quán quân lộ cùng một nhịp — không còn delay ~5s chờ quán quân nhảy; badge + pháo hoa + câu chốt chốt luôn trong bước 05.

= 1.10.13 =
- ket-qua-tong sửa luật trùng điểm: bước lộ hạng mà không còn đội mới (vd đồng hạng 2 nên khuyết hạng 3) giữ nguyên toàn màn hình — tiêu đề twist, chiều cao cột khuyến khích và vòng dao động — thay vì in sai "Hạng 3 · Top 3 đã gọi tên".
- Gom bước 5-6: bấm "Công bố hạng nhì" lộ á quân rồi màn hình tự nhảy quán quân + pháo hoa + câu chốt; không còn nút "Công bố quán quân" riêng. Đồng quán quân (khuyết hạng nhì) nhảy thẳng quán quân.
- Câu chốt ONE DIRECTION ĐỒNG HƯỚNG VƯƠN XA nằm ngay dưới desc; kicker/desc/tiêu đề mr-heading phóng to hơn.
- Sửa snapshot cũ khiến lần diễn tập thứ hai có thể bỏ qua bước lộ hạng của cột.
...
= 1.10.9 =
- Nút tab Phân xe đồng bộ chuẩn nút toàn hệ thống: 14px/700, min-height 44px, radius 10 (bỏ override 12px mobile).
- Nhịp dọc khối Thêm vào xe: heading, ô tên người ngoài và nút "+ Thêm vào xe" cùng margin-top 24px.

= 1.10.8 =
- Khối Thêm vào xe bỏ nền màu, chỉ còn viền ngăn; nút "+ Thêm vào xe" và "Điểm danh lượt mới" dùng gradient đỏ–cam trên desktop giống mobile; nút cách ô nhập 16px hết dính.
