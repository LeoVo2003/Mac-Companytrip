(() => {
  ...
  function totalRevealView() {
    ...
    return `<section class="ma-art-block"><header class="ma-art-block-head"><div><small>TỔNG KẾT COMPANY TRIP</small><h2>Bàn điều khiển công bố</h2></div><a class="ma-screen-link" href="${esc(window.MACVotingAdmin.resultsUrl)}" target="_blank" rel="noopener">↗ Màn hình trình chiếu</a></header><div class="ma-reveal-grid"><section class="ma-panel ma-reveal-control"><header><div><small>TÍN HIỆU TỔNG KẾT</small><h2>Các bước công bố tổng kết</h2></div><span class="ma-reveal-status ${stage.toLowerCase()}"><i></i>${totalRevealStageLabel(stage)}</span></header><div class="ma-reveal-intro"><strong>Năm nhịp · Một cú twist</strong><p>Tung điểm lượn nhẹ → lộ hạng 6-5 (3 ô) → lộ hạng 4-3 (4-5-6 cùng 4 ô, hạng 3 lên 5 ô) và gắn đủ badge 3-4-5-6 → top 2 lên 6 ô, twist bám đuổi → quán quân chạm đỉnh ~82%. Đội trùng điểm nhận đồng hạng và luôn lộ diện cùng nhóm, đồng quán quân được xướng đủ tên. Đội hạng 1-2 trùng điểm được giữ kín tới cú twist.</p></div>${tieWarnings.length ? `<div class="ma-reveal-warn" role="alert"><span>!</span><div><strong>Trùng điểm được phát hiện</strong><ul>${tieWarnings.map((warning) => `<li>${esc(warning)}</li>`).join("")}</ul></div></div>` : ""}<div class="ma-reveal-actions"><button type="button" class="ma-reveal-start" data-total-reveal-stage="ROLLING" ${stage === "IDLE" ? "" : "disabled"}><span>00</span><strong>Mở màn · Tung điểm</strong><small>6 cột lượn điểm nhẹ</small></button><button type="button" data-total-reveal-stage="RANK65" ${stage === "ROLLING" ? "" : "disabled"}><span>01</span><strong>Lộ diện hạng 6 & 5</strong><small>Hai đội đầu tiên · 3 ô</small></button><button type="button" data-total-reveal-stage="RANK43" ${stage === "RANK65" ? "" : "disabled"}><span>02</span><strong>Lộ diện hạng 4 & 3</strong><small>4-5-6 cùng 4 ô · gắn đủ badge 3-4-5-6</small></button><button type="button" data-total-reveal-stage="TWIST" ${stage === "RANK43" || stage === "RANK12" ? "" : "disabled"}><span>03</span><strong>Tạo cú twist</strong><small>Top 2 lên 6 ô · bám đuổi từng điểm</small></button><button type="button" class="ma-reveal-final" data-total-reveal-stage="FINAL" ${stage === "TWIST" ? "" : "disabled"}><span>04</span><strong>Công bố quán quân</strong><small>Chạm đỉnh ~82% + pháo hoa</small></button></div><div class="ma-reveal-footer"><p><i></i>Màn hình trình chiếu tự đồng bộ trong khoảng 1 giây.</p><div class="ma-reveal-footer-actions"><button type="button" id="ma-toggle-scores" class="${scoresHidden ? "is-warn" : ""}">${scoresHidden ? "◎ Hiện điểm trên màn chiếu" : "◉ Ẩn điểm trên màn chiếu"}</button><button type="button" data-total-reveal-stage="IDLE" ${stage === "IDLE" ? "disabled" : ""}>↻ Đặt lại</button></div></div></section><aside class="ma-panel ma-reveal-scoreboard"><header><div><small>CHỈ ADMIN</small><h2>Tổng điểm thật</h2></div></header><div>${rows}</div></aside></div></section>`;
  }
