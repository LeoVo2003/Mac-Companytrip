(() => {
  ...
  function checkinView() {
    ...
    const checkpointCards = (data.checkpoints || []).map((item) => `<article class="ma-check-card ${item.status.toLowerCase()}"><small>MỐC ${item.id} · ${checkpointStatus(item.status)}</small><h2>${esc(item.name)}</h2><p>${esc(item.description || "")}</p><div class="ma-check-actions"><span class="ma-check-fixed">Tối đa 150đ/mốc</span>${canWrite() ? (item.status === "OPEN" ? `<button type="button" class="danger" data-checkpoint="${item.id}" data-op="close">Đóng & chốt</button>` : `${item.status === "DRAFT" ? `<button type="button" class="ma-primary" data-checkpoint="${item.id}" data-op="open">Mở mốc</button>` : `<button type="button" data-checkpoint="${item.id}" data-op="reopen">Mở lại</button>`}<label class="ma-duration"><span>tự đóng sau</span><input type="number" min="1" max="120" value="15" data-checkpoint-duration="${item.id}"><span>phút</span></label>`) : ""}</div></article>`).join("");
    const progressCheckpoints = boards.map((board) => board.checkpoint);
    const progressTeams = boards[0]?.teams || [];
    const progress = `<section class="ma-panel"><header><div><small>TIẾN ĐỘ</small><h2>Điểm theo tỷ lệ có mặt</h2><p style="margin:6px 0 0;color:#667085;font-size:13px">Mỗi team có 15 phút kể từ lượt quét đầu tiên; hết giờ cửa sổ khóa.</p></div>${canWrite() ? `<a class="ma-primary" href="${esc(window.MACVotingAdmin.checkinExportUrl)}">↓ Xuất CSV check-in</a>` : ""}</header><div class="ma-board-table" style="margin:0"><table><thead><tr><th>Đội</th>${progressCheckpoints.map((item) => `<th>Mốc ${item.id} · ${esc(item.name)}</th>`).join("")}</tr></thead><tbody>${progressTeams.map((team) => `<tr><td><strong>#${team.teamNumber} ${esc(team.teamName)}</strong></td>${boards.map((board) => { const cell = board.teams.find((row) => String(row.teamId) === String(team.teamId)); return `<td class="ma-progress-cell">${cell ? `<strong>${cell.checkedIn}/${cell.eligible}</strong><small>${cell.temporaryPoints || 0}đ</small>` : "—"}</td>`; }).join("")}</tr>`).join("") || `<tr><td colspan="${progressCheckpoints.length + 1}">Chưa có mốc nào.</td></tr>`}</tbody></table></div></section>`;
  ...
  function pointsView() {
    ...
    const votePanel = `<section class="ma-panel"><header><div><small>VĂN NGHỆ</small><h2>Quy đổi từ phiếu hợp lệ</h2><p style="margin:6px 0 0;color:#667085;font-size:13px">Điểm = ROUND(TB phiếu ÷ 150 × 200), tối đa 200đ, cập nhật trực tiếp khi có phiếu.</p></div></header><div class="ma-board-table" style="margin:0"><table><thead><tr><th>Team</th><th>TB phiếu</th><th>Số phiếu hợp lệ</th><th>Điểm văn nghệ</th></tr></thead><tbody>${ranked.map((team) => `<tr><td><strong>#${team.teamNumber} ${esc(team.teamName)}</strong></td><td>${team.voteAverage === null ? "Chưa vote" : `${Number(team.voteAverage).toFixed(1)}/150`}</td><td>${team.voteBallots || 0}</td><td><strong>${fmt(team.vote)}</strong></td></tr>`).join("")}</tbody></table></div></section>`;
    ...
    const historyPanel = `<section class="ma-panel"><header><div><small>AUDIT</small><h2>Ai cộng điểm · lúc nào</h2><p style="margin:6px 0 0;color:#667085;font-size:13px">Gồm thi đua, xếp hạng trò chơi và điểm check-in khi đóng mốc.</p></div></header><div class="ma-table ma-history-table"><table><thead><tr><th>Thời gian</th><th>Người cộng</th><th>Team</th><th>Nguồn</th><th>Điểm</th></tr></thead><tbody>${historyRows}</tbody></table></div></section>`;
  ...
  function gamesView() {
    ...
    const ladderChips = `<div class="ma-ladder">${ladder.map((value, index) => `<span class="ma-ladder-chip ${index < 3 ? "rank-" + (index + 1) : ""}">Hạng ${index + 1} · ${value}đ</span>`).join("")}</div>`;
  ...
  function thiduaView() {
    ...
    const award = `<section class="ma-panel ma-award"><div class="ma-award-step"><div class="ma-award-step-head"><h2>1. Lần thi đua</h2><div class="ma-award-cat-actions"><button type="button" id="ma-cat-add" aria-label="Thêm lần thi đua">+</button><button type="button" id="ma-cat-edit" aria-label="Sửa lần thi đua" ${awardCategoryId ? "" : "disabled"}>✎</button><button type="button" id="ma-cat-delete" class="is-danger" aria-label="Xóa lần thi đua" ${awardCategoryId ? "" : "disabled"}>×</button></div></div>${categories.length ? `<select id="ma-award-category">${categories.map((category) => `<option value="${category.id}" ${String(category.id) === String(awardCategoryId) ? "selected" : ""}>${esc(category.name)}</option>`).join("")}</select>` : `<p class="ma-cat-empty">Chưa có lần thi đua. Bấm + để thêm.</p>`}</div><div class="ma-award-step"><h2>2. Team</h2><div class="ma-award-teams">${teams.map((team) => { const score = teamScore(team); const selected = String(team.teamId) === String(awardTeamId); return `<button type="button" data-award-team="${team.teamId}" class="${selected ? "is-selected" : ""}" ${awardCategoryId ? "" : "disabled"} aria-pressed="${selected}"><strong>${esc(team.teamName)}</strong>${score ? `<small>${fmt(score)}</small>` : ""}</button>`; }).join("")}</div></div><div class="ma-award-step"><h2>3. Điểm theo hạng</h2><p style="margin:0 0 8px;color:#667085;font-size:13px">Thang 50 · 40 · 30 · 20 · 10 · 0 (0 = không tham gia). Bấm lại ô đang chọn để xóa.</p><div class="ma-award-presets">${ladder.map((value) => `<button type="button" data-award-points="${value}" class="${canScore && currentPoints === value ? "is-selected" : ""} ${value === 0 ? "is-zero" : ""}" ${canScore ? "" : "disabled"} aria-pressed="${canScore && currentPoints === value}">${value}</button>`).join("")}</div></div></section>`;
    const read = `<section class="ma-panel"><header><div><small>THI ĐUA</small><h2>Điểm từng đội</h2><p style="margin:6px 0 0;color:#667085;font-size:13px">Thang 50 · 40 · 30 · 20 · 10 · 0 cho mỗi lần thi đua. Chỉ super admin mới chấm được.</p></div></header>${thiduaMatrix(categories, teams)}</section>`;
  ...
  function render() {
    ...
    root.querySelectorAll("[data-checkpoint]").forEach((button) => button.addEventListener("click", async () => {
      ...
      const messages = {
        open: `Mở mốc ${button.dataset.checkpoint}? Chỉ một mốc được mở tại một thời điểm và cửa sổ check-in tự khóa sau ${duration} phút.`,
        close: `Đóng & chốt mốc ${button.dataset.checkpoint}? Điểm hạng sẽ được ghi vào sổ.`,
        reopen: `Mở lại mốc ${button.dataset.checkpoint}? Cửa sổ check-in tự khóa sau ${duration} phút.`,
      };
      if (!confirm(messages[operation])) return;
      try {
    ...
    root.querySelector("#ma-exempt-form")?.addEventListener("submit", async (event) => {
      ...
      if (!openCheckpoint) { notify("Chưa có mốc check-in đang mở.", true); return; }
      const form = event.currentTarget;
      const voterId = form.querySelector("#ma-exempt-voter").value;
      const reason = form.querySelector("#ma-exempt-reason").value.trim();
      if (!voterId) { notify("Không còn ai để miễn ở mốc này.", true); return; }
    ...
    root.querySelectorAll("[data-exempt-clear]").forEach((button) => button.addEventListener("click", async () => {
      const openCheckpoint = (data.checkpoints || []).find((item) => item.status === "OPEN");
      if (!openCheckpoint) { notify("Chưa có mốc check-in đang mở.", true); return; }
      if (!confirm(`Bỏ miễn check-in của ${button.dataset.exemptName || "người này"} ở mốc đang mở?`)) return;
      try {
    ...
    root.querySelector("#ma-cat-add")?.addEventListener("click", async () => {
      const name = prompt("Tên lần thi đua mới", "");
      if (name === null) return;
      const trimmed = name.trim();
      if (trimmed.length < 2) { notify("Tên lần thi đua phải có ít nhất 2 ký tự.", true); return; }
      try {
    ...
    root.querySelector("#ma-cat-edit")?.addEventListener("click", async () => {
      ...
      if (!current) return;
      const name = prompt("Đổi tên lần thi đua", current.name);
      if (name === null) return;
      const trimmed = name.trim();
      if (trimmed.length < 2) { notify("Tên lần thi đua phải có ít nhất 2 ký tự.", true); return; }
      try {
    ...
    root.querySelector("#ma-cat-delete")?.addEventListener("click", async () => {
      const current = (data.totalBoard?.categories || []).find((category) => String(category.id) === String(awardCategoryId));
      if (!current) return;
      if (!confirm(`Xóa lần thi đua “${current.name}”? Điểm đã cộng của lần thi đua này cũng sẽ mất.`)) return;
      try {
