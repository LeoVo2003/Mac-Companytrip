(() => {
  ...
  function busView() {
    ...
    const control = `<section class="ma-panel ma-bus-control">
      <header><div><small>PHÂN XE · TRẠM 1</small><h2>${boarding ? `Đang xếp ${esc(boarding.name)} · ${boarding.total}/${boarding.capacity} chỗ` : "Chờ lượt quét Trạm 1 đầu tiên"}</h2><p style="margin:6px 0 0;color:#667085;font-size:13px">Quét một QR sẽ xếp cả nhóm người thân lên cùng xe. Nếu xe không đủ chỗ cho cả nhóm, hệ thống chuyển trọn nhóm sang xe kế tiếp.</p></div><div class="ma-bus-export-all"><span class="ma-reveal-status ${boarding ? "rolling" : "idle"}"><i></i>${state.enabled ? (boarding ? "ĐANG PHÂN XE" : "CHỜ QUÉT ĐẦU TIÊN") : "CHƯA BẬT / ĐÃ HOÀN TẤT"}</span>${canWrite() ? `<a class="ma-bus-ghost" href="${esc(window.MACVotingAdmin.allBusesExportUrl)}" title="Xuất danh sách tổng gửi resort — Super Admin xuất được mọi lúc">↓ Xuất tổng 5 xe</a>` : ""}</div></header>
