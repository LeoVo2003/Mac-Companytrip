(() => {
  ...
  // BTC/Hoa tiêu + Super Admin cùng chấm thi đua, xếp game; HDV chỉ xem.
  const canOperate = () => window.MACVotingAdmin.role !== "guide";
  let tab = "overview";
  ...
  async function previewImport(file) {
    ...
    checking.innerHTML = `<span></span><strong>Đang đọc dữ liệu và ghép nhóm người thân / chung phòng…</strong>`;
    ...
    const headers = serverValidation?.headers || [];
    const previewRows = serverValidation?.previewRows || [];
    const totalRows = Number(serverValidation?.inserted || 0) + Number(serverValidation?.updated || 0);
    const canImport = !serverError && totalRows > 0;
    const modal = document.createElement("div");
    modal.className = "ma-modal";
    const validationMessage = serverError
      ? `<div class="ma-import-warning"><strong>File CSV chưa hợp lệ</strong><span>${esc(serverError)}</span></div>`
      : `<div class="ma-import-ready"><strong>✓ ${totalRows} người hợp lệ</strong><span>${esc(serverValidation?.message || "Dữ liệu đã được kiểm tra.")}</span><small>${Number(serverValidation?.companions || 0)} người đi kèm · ${Number(serverValidation?.familyGroups || 0)} nhóm người thân · ${Number(serverValidation?.roomGroups || 0)} nhóm chung phòng · ${Number(serverValidation?.nonBusRiders || 0)} người không đi xe</small></div>`;
    modal.innerHTML = `<div class="ma-modal-card" role="dialog" aria-modal="true" aria-labelledby="ma-import-title"><div class="ma-modal-head"><div><small>XEM TRƯỚC DỮ LIỆU</small><h2 id="ma-import-title">Xác nhận import CSV</h2></div><button type="button" id="ma-import-close" aria-label="Đóng">×</button></div><div class="ma-file-summary"><span>CSV</span><div><strong>${esc(file.name)}</strong><small>${formatBytes(file.size)} · ghép nhóm theo cột Note (người thân N) và Phòng cùng giá trị</small></div></div>${validationMessage}<div class="ma-preview-count">Đang hiển thị <strong>${previewRows.length}/${totalRows}</strong> người</div><div class="ma-preview-table"><table><thead><tr><th>#</th>${headers.slice(0, 12).map((value) => `<th>${esc(value)}</th>`).join("")}</tr></thead><tbody>${previewRows.map((values, rowIndex) => `<tr><td>${rowIndex + 1}</td>${headers.slice(0, 12).map((_, index) => `<td>${esc(values[index] || "")}</td>`).join("")}</tr>`).join("") || `<tr><td colspan="${Math.max(headers.length + 1, 2)}">Không có dòng dữ liệu.</td></tr>`}</tbody></table></div><div class="ma-modal-actions"><button type="button" id="ma-import-cancel">Hủy</button><button type="button" id="ma-import-confirm" class="ma-primary" ${canImport ? "" : "disabled"}>Xác nhận import ${totalRows} người</button></div></div>`;
    root.append(modal);
  ...
  const resetModules = {
    checkin: { action: "mac_vote_reset_checkin", label: "Đặt lại Check-in", title: "Đặt lại Check-in?", message: "Trạm, lượt quét, miễn check-in và điểm Check-in sẽ được xóa. Văn nghệ, Trò chơi lớn, Thi đua và Phân xe vẫn giữ nguyên." },
    bus: { action: "mac_vote_bus_reset", label: "Đặt lại Phân xe", title: "Đặt lại Phân xe?", message: "Danh sách xe, người đã gán và lượt điểm danh trên xe sẽ bị xóa; 5 xe về trạng thái chờ. Check-in và điểm thi đấu không bị ảnh hưởng." },
    games: { action: "mac_vote_reset_games", label: "Đặt lại Trò chơi lớn", title: "Đặt lại điểm Trò chơi lớn?", message: "Toàn bộ hạng và điểm của 3 game sẽ được xóa. Tên các game, Check-in, Văn nghệ và Thi đua vẫn giữ nguyên." },
    art: { action: "mac_vote_reset_art", label: "Đặt lại Văn nghệ", title: "Đặt lại Văn nghệ?", message: "Toàn bộ phiếu, quyền vote lại và trạng thái công bố sẽ được xóa; cổng chấm điểm được tắt. Team và lịch biểu diễn vẫn giữ nguyên." },
    thidua: { action: "mac_vote_reset_thidua", label: "Đặt lại Thi đua", title: "Đặt lại điểm Thi đua?", message: "Điểm của mọi đội trong các hạng mục Thi đua sẽ được xóa. Tên các hạng mục vẫn giữ nguyên." },
  };
  const topActions = (scope = "") => {
    const module = resetModules[scope];
    return `<div class="ma-top-actions"><button id="ma-refresh">↻ Tải lại dữ liệu</button>${canWrite() && module ? `<button type="button" class="ma-reset-trigger ma-module-reset" data-module-reset="${scope}">${module.label}</button>` : ""}${canWrite() && scope === "event" ? `<button id="ma-reset-event" class="ma-reset-trigger">Đặt lại sự kiện</button>` : ""}</div>`;
  };
  ...
  function teamManager() {
    ...
    return `<section class="ma-panel ma-team-manager"><header><div><small>QUẢN LÝ TEAM</small><h2>Danh sách team linh hoạt</h2><p>Đổi tên trực tiếp hoặc thêm team mới. Chương trình vẫn có 6 slot; có thể chuyển nhân sự bằng cách import lại XLSX rồi chọn team biểu diễn trong lịch.</p></div><form id="ma-team-add"><label for="ma-new-team-name">Tên team mới</label><div><input id="ma-new-team-name" name="name" type="text" minlength="2" maxlength="100" placeholder="Ví dụ: Sóng Biển" required><button class="ma-primary" type="submit">+ Thêm team</button></div></form></header><div class="ma-team-list">${teams.map((team) => {
  ...
  const totalRevealStageLabel = (stage) => ({ IDLE: "Đang chờ", ROLLING: "Đang tung điểm", RANK65: "Đã lộ diện hạng 6-5", TEASE43: "Đang nhá hàng top 4", RANK43: "Đã lộ diện hạng 4", RANK12: "Top 2 đã bước lên", TWIST: "3 đội đang cùng tung điểm", REVEAL3: "Đã lộ diện hạng 3", SECOND: "Đã công bố á quân + quán quân", FINAL: "Đã công bố quán quân" }[stage] || "Đang chờ");
  function sidebar() {
  ...
  function checkinView() {
    ...
    return `<header class="ma-top"><div><small>CHECK-IN</small><h1>4 trạm Company Trip</h1></div>${topActions("checkin")}</header>${scanner}<div class="ma-check-grid">${checkpointCards}</div>${progress}${exemptionPanel}${staffPanel}`;
  }
  function busView() {
    const state = data.buses || { buses: [], unassigned: [], guides: [] };
    const buses = state.buses || [];
    const allBusesClosed = buses.length === 5 && buses.every((bus) => bus.status === "CLOSED");
    ...
    const control = `<section class="ma-panel ma-bus-control">
      <header><div><small>PHÂN XE · TRẠM 1</small><h2>${boarding ? `Đang xếp ${esc(boarding.name)} · ${boarding.total}/${boarding.capacity} chỗ` : "Chờ lượt quét Trạm 1 đầu tiên"}</h2><p style="margin:6px 0 0;color:#667085;font-size:13px">Quét một QR sẽ xếp cả nhóm người thân lên cùng xe. Nếu xe không đủ chỗ cho cả nhóm, hệ thống chuyển trọn nhóm sang xe kế tiếp.</p></div><div class="ma-bus-export-all"><span class="ma-reveal-status ${boarding ? "rolling" : "idle"}"><i></i>${state.enabled ? (boarding ? "ĐANG PHÂN XE" : "CHỜ QUÉT ĐẦU TIÊN") : "CHƯA BẬT / ĐÃ HOÀN TẤT"}</span>${canWrite() ? `<a class="ma-bus-ghost ${allBusesClosed ? "" : "is-disabled"}" ${allBusesClosed ? `href="${esc(window.MACVotingAdmin.allBusesExportUrl)}"` : `aria-disabled="true" title="Chỉ xuất sau khi đã đóng đủ 5 xe"`}> ↓ Xuất tổng 5 xe</a>` : ""}</div></header>
    ...
    const anyPool = canWrite() ? (data.voters || []).filter((v) => v.status === "ACTIVE" && !assignedIds.has(Number(v.id)) && (!anyQueryNorm || `${v.full_name} ${v.team_name || ""}`.toLowerCase().includes(anyQueryNorm))).sort((a, b) => String(a.full_name).localeCompare(String(b.full_name), "vi")).slice(0, 60) : [];
    const q = busQuery.trim().toLowerCase();
    const members = ((manifestBus?.manifest) || []).filter((m) => !q || `${m.name} ${m.teamName || ""}`.toLowerCase().includes(q));
    const memberRows = members.map((m) => `<tr><td><strong>${esc(m.name)}</strong>${m.memberType === "COMPANION" ? `<small class="ma-cell-sub">Người đi kèm</small>` : ""}</td><td>${m.teamNo ? `#${m.teamNo} ${esc(m.teamName)}` : "—"}</td><td>${m.memberType === "EMPLOYEE" ? "NV QR" : m.memberType === "COMPANION" ? "Người đi kèm" : m.memberType === "STAFF" ? "BTC/Hoa tiêu" : "Thủ công"}</td><td class="ma-bus-actions">${busStaff ? `<button type="button" class="ma-roll-mini ${roll.marks[m.id] ? "is-present" : ""}" data-rollcall-toggle="${m.id}" data-bus="${busManifestId}" data-present="${roll.marks[m.id] ? "" : "1"}" aria-pressed="${!!roll.marks[m.id]}" title="Điểm danh">${roll.marks[m.id] ? "✓" : "○"}</button>` : ""}${canWrite() ? `<select data-bus-move-member="${m.id}" aria-label="Chuyển cả nhóm của ${esc(m.name)}"><option value="">Chuyển cả nhóm…</option>${buses.filter((b) => b.id !== busManifestId).map((b) => `<option value="${b.id}">${esc(b.name)}</option>`).join("")}</select><button type="button" class="danger ma-bus-remove" data-bus-remove="${m.id}" data-bus-remove-name="${esc(m.name)}" aria-label="Xóa nhóm ${esc(m.name)} khỏi xe">×</button>` : ""}</td></tr>`).join("") || `<tr><td colspan="4" class="ma-cell-empty">${q ? "Không ai khớp tìm kiếm." : "Xe chưa có ai."}</td></tr>`;
    const manifest = manifestBus ? `<section class="ma-panel ma-bus-manifest"><div class="ma-bus-tabs" role="tablist" aria-label="Chọn xe">${buses.map((b) => `<button type="button" role="tab" data-bus-manifest="${b.id}" class="${b.id === busManifestId ? "active" : ""}" aria-selected="${b.id === busManifestId}"><strong>${esc(b.name)}</strong><small>${b.total}</small></button>`).join("")}</div><header><div><small>MANIFEST · LƯỢT ${roll.sequence || "—"}</small><h2>${esc(manifestBus.name)} · ${roll.presentCount}/${manifestBus.total} có mặt</h2></div><div class="ma-bus-tools"><input id="ma-bus-query" type="search" placeholder="Tìm tên / team" value="${esc(busQuery)}"><a href="${esc(window.MACVotingAdmin.busExportUrl)}&bus_id=${manifestBus.id}" class="ma-bus-ghost">↓ Xuất XLSX xe</a>${(canWrite() || window.MACVotingAdmin.role === "guide") ? `<button type="button" class="ma-primary" id="ma-rollcall-new" data-bus="${busManifestId}">Điểm danh lượt mới</button>` : ""}</div></header><div class="ma-board-table ma-no-sticky"><table><thead><tr><th>Họ tên</th><th>Team</th><th>Loại</th><th></th></tr></thead><tbody>${memberRows}</tbody></table></div>${busStaff && roll.history.length ? `<div class="ma-bus-history"><small>LỊCH SỬ LƯỢT ĐIỂM DANH</small><ul>${roll.history.map((h) => `<li>Lượt ${h.sequence} · ${esc(h.createdAt)} · ${h.presentCount}/${manifestBus.total}</li>`).join("")}</ul></div>` : ""}${busStaff ? `<form class="ma-bus-add" id="ma-bus-add-form"><div class="ma-bus-add-group"><div class="ma-bus-add-head"><span>Thêm BTC/Hoa tiêu vào ${esc(manifestBus.name)}</span><small>Chọn nhiều người cùng lúc rồi bấm thêm — người đã ở xe khác sẽ không xuất hiện</small></div><div class="ma-bus-pick">${pickHtml}</div></div>${canWrite() ? `<div class="ma-bus-add-group"><div class="ma-bus-add-head"><span>Thêm nhân sự bất kỳ</span><small>Tìm tên, tick rồi thêm; nhóm người thân sẽ đi cùng nhau</small></div><input id="ma-bus-any-query" type="search" placeholder="Tìm tên để thêm thủ công…" value="${esc(busAnyQuery)}"><div class="ma-bus-pick">${anyPool.map((v) => `<label class="ma-pick-item"><input type="checkbox" name="anyIds" value="${v.id}"> ${esc(v.full_name)} <small>#${v.team_no}</small></label>`).join("") || `<p class="ma-pick-empty">Gõ tên để tìm người chưa ở xe nào.</p>`}</div><input id="ma-bus-manual" type="text" placeholder="Hoặc gõ tên người ngoài (tùy chọn)"></div>` : ""}<button type="submit" class="ma-primary">+ Thêm vào xe</button></form>` : ""}</section>` : "";
    ...
    const guideRows = (state.guides || []).map((g) => `<tr><td><strong>${esc(g.name)}</strong></td><td>${esc(g.login)}</td><td><select data-guide-bus="${g.id}" aria-label="Đổi xe phụ trách của ${esc(g.name)}">${[1, 2, 3, 4, 5].map((n) => `<option value="${n}" ${g.busId === n ? "selected" : ""}>Xe ${n}</option>`).join("")}</select></td><td class="ma-bus-actions"><button type="button" class="ma-bus-ghost-danger" data-guide-delete="${g.id}" data-guide-name="${esc(g.name)}">Xóa</button></td></tr>`).join("") || `<tr><td colspan="4">Chưa có tài khoản HDV.</td></tr>`;
    const guides = `<section class="ma-panel"><header><div><small>HDV VIETRAVEL</small><h2>${(state.guides || []).length} tài khoản điểm danh trên xe</h2><p style="margin:6px 0 0;color:#667085;font-size:13px">HDV chỉ thấy Check-in + Xe của tôi; quét QR được mọi team nhưng chỉ điểm danh xe mình. Đổi xe không đổi mật khẩu; xóa là mất quyền đăng nhập ngay.</p></div></header><form class="ma-guide-form" id="ma-guide-form"><label>Tên hiển thị<input name="name" type="text" placeholder="VD: Anh Tuấn Vietravel" required></label><label>Username<input name="login" type="text" placeholder="hdv.xe1" required autocapitalize="none"></label><label>Mật khẩu<input name="password" type="text" placeholder="Để trống = Mac-123"></label><label>Xe phụ trách<select name="busId">${[1, 2, 3, 4, 5].map((n) => `<option value="${n}">Xe ${n}</option>`).join("")}</select></label><button type="submit" class="ma-primary">Lưu tài khoản HDV</button></form><div class="ma-board-table ma-no-sticky"><table><thead><tr><th>HDV</th><th>Đăng nhập</th><th>Xe phụ trách</th><th></th></tr></thead><tbody>${guideRows}</tbody></table></div></section>`;
    const selfArrange = state.selfArrange || [];
    const selfPanel = `<section class="ma-panel"><header><div><small>TỰ TÚC · KHÔNG ĐI XE TRẠM 1</small><h2>${selfArrange.length} người chờ gán tay</h2><p style="margin:6px 0 0;color:#667085;font-size:13px">Người đánh dấu Đi xe = Không khi import. Sau khi chốt phân xe Trạm 1, Super Admin gán họ vào xe phù hợp; gán người chính sẽ kéo theo cả nhóm đi kèm.</p></div></header><div class="ma-board-table ma-no-sticky"><table><thead><tr><th>Họ tên</th><th>Team</th><th>Đi kèm</th><th>Gán vào xe</th></tr></thead><tbody>${selfArrange.map((u) => `<tr><td><strong>${esc(u.name)}</strong></td><td>#${u.teamNo} ${esc(u.teamName)}</td><td>${u.companions ? `<details class="ma-companions"><summary><span class="ma-companions-count">+${u.companions}</span><span>đi kèm</span></summary><div class="ma-companions-pop"><ul>${(u.companionNames || []).map((n) => `<li>${esc(n)}</li>`).join("")}</ul></div></details>` : `<span class="ma-companions-none">—</span>`}</td><td><select data-self-assign="${u.voterId}" aria-label="Gán ${esc(u.name)} vào xe"><option value="">Chọn xe…</option>${buses.map((b) => `<option value="${b.id}">${esc(b.name)}</option>`).join("")}</select></td></tr>`).join("") || `<tr><td colspan="4">Hết người tự túc chờ gán.</td></tr>`}</tbody></table></div></section>`;
  ...
  function pointsView() {
    ...
    return `<header class="ma-top"><div><small>TỔNG QUAN</small><h1>Tổng điểm 6 team</h1></div>${topActions("event")}</header>${nav}${body}`;
  }
  function gamesView() {
    const games = data.games?.list || [];
    const gameBoard = data.games?.board || [];
  ...
  function thiduaView() {
    ...
    return `<header class="ma-top"><div><small>THI ĐUA</small><h1>Chấm điểm thi đua</h1></div>${topActions("thidua")}</header>${canOperate() ? award : read}`;
  }
  function live() {
  ...
  function totalRevealView() {
    ...
    return `<section class="ma-art-block"><header class="ma-art-block-head"><div><small>TỔNG KẾT COMPANY TRIP</small><h2>Bàn điều khiển công bố</h2></div><a class="ma-screen-link" href="${esc(window.MACVotingAdmin.resultsUrl)}" target="_blank" rel="noopener">↗ Màn hình trình chiếu</a></header><div class="ma-reveal-grid"><section class="ma-panel ma-reveal-control"><header><div><small>TÍN HIỆU TỔNG KẾT</small><h2>Các bước công bố tổng kết</h2></div><span class="ma-reveal-status ${stage.toLowerCase()}"><i></i>${totalRevealStageLabel(stage)}</span></header><div class="ma-reveal-intro"><strong>Công bố tổng kết</strong><p>Công bố kết quả chung cuộc · 6 đội · 4 mặt trận.</p></div>${tieWarnings.length ? `<div class="ma-reveal-warn" role="alert"><span>!</span><div><strong>Trùng điểm được phát hiện</strong><ul>${tieWarnings.map((warning) => `<li>${esc(warning)}</li>`).join("")}</ul></div></div>` : ""}<div class="ma-reveal-actions"${canWrite() ? "" : " hidden"}><button type="button" class="ma-reveal-start" data-total-reveal-stage="ROLLING" ${stage === "IDLE" ? "" : "disabled"}><span>00</span><strong>Mở màn · Tung điểm</strong><small>Kéo mượt từ vạch 122px</small></button><button type="button" data-total-reveal-stage="RANK65" ${stage === "ROLLING" ? "" : "disabled"}><span>01</span><strong>Lộ diện hạng 6 & 5</strong><small>80% · badge khuyến khích</small></button><button type="button" data-total-reveal-stage="${stage === "TEASE43" ? "RANK43" : "TEASE43"}" ${stage === "RANK65" || stage === "TEASE43" ? "" : "disabled"}><span>02</span><strong>${stage === "TEASE43" ? "Hiện hạng 4" : "Nhá hàng top 4"}</strong><small>${stage === "TEASE43" ? "Chỉ lộ hạng 4-5-6 · badge khuyến khích" : "Nhấn lần 1: nhấp nháy · lần 2: lộ diện"}</small></button><button type="button" data-total-reveal-stage="TWIST" ${stage === "RANK43" || stage === "RANK12" ? "" : "disabled"}><span>03</span><strong>Tạo cú twist</strong><small>3 đội dẫn đầu cùng tung điểm</small></button><button type="button" data-total-reveal-stage="REVEAL3" ${stage === "TWIST" ? "" : "disabled"}><span>04</span><strong>Công bố hạng 3</strong><small>Hạng ba lộ diện · 1-2 tung tiếp</small></button><button type="button" class="ma-reveal-final" data-total-reveal-stage="SECOND" ${stage === "REVEAL3" ? "" : "disabled"}><span>05</span><strong>Công bố hạng nhì</strong><small>Á quân + quán quân cùng nhịp · pháo hoa khép lại</small></button></div><div class="ma-reveal-footer"><p><i></i>Màn hình trình chiếu tự đồng bộ trong khoảng 1 giây.</p><div class="ma-reveal-footer-actions"${canWrite() ? "" : " hidden"}><button type="button" id="ma-toggle-scores" class="${scoresHidden ? "is-warn" : ""}">${scoresHidden ? "◎ Hiện điểm trên màn chiếu" : "◉ Ẩn điểm trên màn chiếu"}</button><button type="button" data-total-reveal-stage="IDLE" ${stage === "IDLE" ? "disabled" : ""}>↻ Đặt lại</button></div></div></section><aside class="ma-panel ma-reveal-scoreboard"><header><div><small>CHỈ ADMIN</small><h2>Tổng điểm thật</h2></div></header><div>${rows}</div></aside></div></section>`;
  }
  ...
  function render() {
    ...
    root.querySelectorAll("[data-module-reset]").forEach((button) => button.addEventListener("click", async () => {
      ...
      const module = resetModules[scope];
      if (!module || !(await confirmDialog({ title: module.title, message: module.message, confirmLabel: module.label, danger: true }))) return;
      const originalLabel = button.textContent;
      button.disabled = true;
      button.classList.add("is-loading");
      button.textContent = "Đang đặt lại…";
      try {
        const result = await ajax(module.action);
        data = result.overview;
        if (scope === "bus") busManifestId = 0;
        render();
        notify(result.message);
      } catch (err) {
    ...
    root.querySelectorAll("[data-guide-bus]").forEach((select) => select.addEventListener("change", async () => {
      select.disabled = true;
      try {
        const result = await ajax("mac_vote_guide_bus", { userId: select.dataset.guideBus, busId: select.value });
        data = result.overview;
        render();
        notify(result.message);
      } catch (err) { select.disabled = false; notify(err.message, true); }
    }));
    root.querySelectorAll("[data-guide-delete]").forEach((button) => button.addEventListener("click", async () => {
      if (!(await confirmDialog({ title: "Xóa tài khoản HDV?", message: `Xóa vĩnh viễn tài khoản ${button.dataset.guideName}? Người này sẽ không đăng nhập máy quét / điểm danh được nữa.`, confirmLabel: "Xóa tài khoản", danger: true }))) return;
      try {
        const result = await ajax("mac_vote_guide_delete", { userId: button.dataset.guideDelete });
        data = result.overview;
        render();
        notify(result.message);
      } catch (err) { notify(err.message, true); }
    ...
    root.querySelectorAll("[data-busrider-toggle]").forEach((button) => button.addEventListener("click", async () => {
      const current = Number(button.dataset.busrider ?? 1);
      const next = current === 0 ? 1 : 0;
      if (next === 0 && !(await confirmDialog({ title: "Đánh dấu tự túc?", message: "Người này sẽ không được tự động xếp xe khi quét QR Trạm 1 mà nằm trong bảng TỰ TÚC để gán tay. Vẫn có tên trong file gửi resort nếu cột Resort để trống.", confirmLabel: "Đánh dấu tự túc", danger: true }))) return;
      button.disabled = true;
      try {
        const result = await ajax("mac_vote_person_bus_rider", { voterId: button.dataset.busriderToggle, busRider: next });
        data = result.overview;
        render();
        notify(result.message);
