(() => {
  ...
  function render() {
    ...
    root.querySelectorAll("[data-checkpoint]").forEach((button) => button.addEventListener("click", async () => {
      ...
      const checkpointLabels = { open: "Mở mốc", close: "Đóng & chốt", reopen: "Mở lại" };
      if (!(await confirmDialog({ title: `Mốc ${button.dataset.checkpoint}`, message: messages[operation], confirmLabel: checkpointLabels[operation], danger: operation === "close" }))) return;
      try {
    ...
    root.querySelectorAll("[data-exempt-clear]").forEach((button) => button.addEventListener("click", async () => {
      const openCheckpoint = (data.checkpoints || []).find((item) => item.status === "OPEN");
      if (!openCheckpoint) { notify("Chưa có mốc check-in đang mở.", true); return; }
      if (!(await confirmDialog({ title: "Bỏ miễn check-in", message: `Bỏ miễn check-in của ${button.dataset.exemptName || "người này"} ở mốc đang mở?`, confirmLabel: "Bỏ miễn", danger: true }))) return;
      try {
    ...
    root.querySelector("#ma-cat-delete")?.addEventListener("click", async () => {
      const current = (data.totalBoard?.categories || []).find((category) => String(category.id) === String(awardCategoryId));
      if (!current) return;
      if (!(await confirmDialog({ title: "Xóa lần thi đua", message: `Xóa lần thi đua “${current.name}”? Điểm đã cộng của lần thi đua này cũng sẽ mất.`, confirmLabel: "Xóa lần thi đua", danger: true }))) return;
      try {
