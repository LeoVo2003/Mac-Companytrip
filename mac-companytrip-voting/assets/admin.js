(() => {
  ...
  const revealStageLabel = (stage) => ({ IDLE: "Đang chờ", ROLLING: "Đang tung điểm", DECOY: "Đã chốt cú lừa", THIRD: "Đã công bố hạng ba", SECOND: "Đã công bố hạng nhì", FINAL: "Đã công bố quán quân" }[stage] || "Đang chờ");
    const totalRevealStageLabel = (stage) => ({ IDLE: "Đang chờ", ROLLING: "Đang tung điểm", RANK65: "Đã lộ diện hạng 6-5", TEASE43: "Đang nhá hàng top 4", RANK43: "Đã lộ diện hạng 4-3", RANK12: "Top 2 đã bước lên", TWIST: "Đang tạo cú twist", FINAL: "Đã công bố quán quân" }[stage] || "Đang chờ");
  ...
  function totalRevealView() {
    ...
    return `<section class="ma-art-block"><header class="ma-art-block-head"><div><small>TỔNG KẾT COMPANY TRIP</small><h2>Bàn điều khiển công bố</h2></div><a class="ma-screen-link" href="${esc(window.MACVotingAdmin.resultsUrl)}" target="_blank" rel="noopener">↗ Màn hình trình chiếu</a></header><div class="ma-reveal-grid"><section class="ma-panel ma-reveal-control"><header><div><small>TÍN HIỆU TỔNG KẾT</small><h2>Các bước công bố tổng kết</h2></div><span class="ma-reveal-status ${stage.toLowerCase()}"><i></i>${totalRevealStageLabel(stage)}</span></header><div class="ma-reveal-intro"><strong>Năm nhịp · Một cú twist</strong><p>Tung điểm kéo mượt từ vạch 122px → lộ hạng 6-5 (80%) → nhá hàng top 4 (nhấn lần 1: nhấp nháy, nhấn lần 2: lộ 4-5-6 cùng 50% + hạng 3 lên 80%, gắn đủ badge 3-4-5-6) → twist: top 2 dao động 70-90%, hạng 3 về 50%, hạng 4-5-6 về 30% → quán quân 85%, hạng nhì 60%. Đội trùng điểm nhận đồng hạng, lộ cùng nhóm; đội hạng 1-2 trùng điểm giữ kín tới cú twist.</p></div>${tieWarnings.length ? `<div class="ma-reveal-warn" role="alert"><span>!</span><div><strong>Trùng điểm được phát hiện</strong><ul>${tieWarnings.map((warning) => `<li>${esc(warning)}</li>`).join("")}</ul></div></div>` : ""}<div class="ma-reveal-actions"><button type="button" class="ma-reveal-start" data-total-reveal-stage="ROLLING" ${stage === "IDLE" ? "" : "disabled"}><span>00</span><strong>Mở màn · Tung điểm</strong><small>Kéo mượt từ vạch 122px</small></button><button type="button" data-total-reveal-stage="RANK65" ${stage === "ROLLING" ? "" : "disabled"}><span>01</span><strong>Lộ diện hạng 6 & 5</strong><small>Hai đội đầu tiên · 80%</small></button><button type="button" data-total-reveal-stage="${stage === "TEASE43" ? "RANK43" : "TEASE43"}" ${stage === "RANK65" || stage === "TEASE43" ? "" : "disabled"}><span>02</span><strong>${stage === "TEASE43" ? "Lộ diện top 4" : "Nhá hàng top 4"}</strong><small>${stage === "TEASE43" ? "Nhấn để lộ hạng 4-3 · 4-5-6 cùng 50%" : "Nhấn lần 1: nhấp nháy · lần 2: lộ diện"}</small></button><button type="button" data-total-reveal-stage="TWIST" ${stage === "RANK43" || stage === "RANK12" ? "" : "disabled"}><span>03</span><strong>Tạo cú twist</strong><small>Top 2 dao động 70-90%</small></button><button type="button" class="ma-reveal-final" data-total-reveal-stage="FINAL" ${stage === "TWIST" ? "" : "disabled"}><span>04</span><strong>Công bố quán quân</strong><small>Nhất 85% · Nhì 60% + pháo hoa</small></button></div><div class="ma-reveal-footer"><p><i></i>Màn hình trình chiếu tự đồng bộ trong khoảng 1 giây.</p><div class="ma-reveal-footer-actions"><button type="button" id="ma-toggle-scores" class="${scoresHidden ? "is-warn" : ""}">${scoresHidden ? "◎ Hiện điểm trên màn chiếu" : "◉ Ẩn điểm trên màn chiếu"}</button><button type="button" data-total-reveal-stage="IDLE" ${stage === "IDLE" ? "disabled" : ""}>↻ Đặt lại</button></div></div></section><aside class="ma-panel ma-reveal-scoreboard"><header><div><small>CHỈ ADMIN</small><h2>Tổng điểm thật</h2></div></header><div>${rows}</div></aside></div></section>`;
  }
