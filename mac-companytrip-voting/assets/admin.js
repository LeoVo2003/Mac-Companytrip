(() => {
  ...
  async function previewImport(file) {
    ...
    const rowErrors = missing.length ? [] : previewRows.flatMap((values, index) => {
      ...
      if (!/^[^\s@]+@(macusaone\.com|yesoffice\.vn)$/i.test(email)) issues.push("email phải thuộc @macusaone.com hoặc @yesoffice.vn");
