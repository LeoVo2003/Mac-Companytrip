(() => {
  ...
  async function previewImport(file) {
    ...
    let serverError = "";
    if (!/\.xlsx$/i.test(file.name)) { notify("Chỉ chấp nhận file XLSX.", true); return; }
    if (file.size > 5 * 1024 * 1024) { notify("File XLSX không được lớn hơn 5 MB.", true); return; }
    const checking = document.createElement("div");
    checking.className = "ma-import-checking";
    checking.setAttribute("role", "status");
    checking.innerHTML = `<span></span><strong>Đang đọc dữ liệu và các ô đã gộp…</strong>`;
    ...
    const validationMessage = serverError
      ? `<div class="ma-import-warning"><strong>File XLSX chưa hợp lệ</strong><span>${esc(serverError)}</span></div>`
    ...
    modal.innerHTML = `<div class="ma-modal-card" role="dialog" aria-modal="true" aria-labelledby="ma-import-title"><div class="ma-modal-head"><div><small>XEM TRƯỚC DỮ LIỆU</small><h2 id="ma-import-title">Xác nhận import XLSX</h2></div><button type="button" id="ma-import-close" aria-label="Đóng">×</button></div><div class="ma-file-summary"><span>XLSX</span><div><strong>${esc(file.name)}</strong><small>${formatBytes(file.size)} · đã đọc cả ô gộp phòng và người thân</small></div></div>${validationMessage}<div class="ma-preview-count">Đang hiển thị <strong>${previewRows.length}/${totalRows}</strong> người</div><div class="ma-preview-table"><table><thead><tr><th>#</th>${headers.slice(0, 10).map((value) => `<th>${esc(value)}</th>`).join("")}</tr></thead><tbody>${previewRows.map((values, rowIndex) => `<tr><td>${rowIndex + 1}</td>${headers.slice(0, 10).map((_, index) => `<td>${esc(values[index] || "")}</td>`).join("")}</tr>`).join("") || `<tr><td colspan="${Math.max(headers.length + 1, 2)}">Không có dòng dữ liệu.</td></tr>`}</tbody></table></div><div class="ma-modal-actions"><button type="button" id="ma-import-cancel">Hủy</button><button type="button" id="ma-import-confirm" class="ma-primary" ${canImport ? "" : "disabled"}>Xác nhận import ${totalRows} người</button></div></div>`;
    root.append(modal);
