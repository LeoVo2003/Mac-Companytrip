(() => {
  ...
  const revealStageLabel = (stage) => ({ IDLE: "Đang chờ", ROLLING: "Đang xuất phát", DECOY: "Đang cú lừa", RANK65: "Đã lộ diện hạng 6-5", RANK43: "Đã lộ diện hạng 4-3", TWIST: "Hai thuyền đang bám đuổi", THIRD: "Đã công bố hạng ba", SECOND: "Đã công bố hạng nhì", FINAL: "Quán quân đã về đích" }[stage] || "Đang chờ");
  ...
  function revealView() {
    ...
    return `<section class="ma-art-block"><header class="ma-art-block-head"><div><small>CÔNG BỐ</small><h2>Đua thuyền văn nghệ</h2></div><a class="ma-screen-link" href="${esc(window.MACVotingAdmin.resultsUrl)}" target="_blank" rel="noopener">↗ Màn đua thuyền</a></header><div class="ma-reveal-grid"><section class="ma-panel ma-reveal-control"><header><div><small>LIVE REVEAL</small><h2>Tín hiệu MC</h2></div><span class="ma-reveal-status ${stage.toLowerCase()}"><i></i>${revealStageLabel(stage)}</span></header><div class="ma-reveal-intro"><strong>Một cú lừa · Đường đua 6 làn</strong><p>Xuất phát nhấp nhô → cú lừa: 3 thuyền cuối bảng giả dẫn đầu → hạ màn lừa lộ hạng 6-5 (badge KHUYẾN KHÍCH) → lộ hạng 4-3 → cú twist: 2 thuyền bám đuổi → quán quân băng vạch đích + pháo hoa.</p></div><div class="ma-reveal-actions"><button type="button" class="ma-reveal-start" data-reveal-stage="ROLLING" ${stage === "IDLE" ? "" : "disabled"}><span>00</span><strong>Xuất phát</strong><small>6 thuyền nhấp nhô ra khơi</small></button><button type="button" data-reveal-stage="DECOY" ${stage === "ROLLING" ? "" : "disabled"}><span>01</span><strong>Cú lừa</strong><small>3 thuyền cuối bảng giả dẫn đầu</small></button><button type="button" data-reveal-stage="RANK65" ${stage === "DECOY" ? "" : "disabled"}><span>02</span><strong>Hạ màn lừa · lộ 6-5</strong><small>Badge KHUYẾN KHÍCH</small></button><button type="button" data-reveal-stage="RANK43" ${stage === "RANK65" ? "" : "disabled"}><span>03</span><strong>Lộ hạng 4 & 3</strong><small>Đường đua dần rõ</small></button><button type="button" data-reveal-stage="TWIST" ${stage === "RANK43" || stage === "THIRD" ? "" : "disabled"}><span>04</span><strong>Cú twist</strong><small>2 thuyền bám đuổi từng sóng</small></button><button type="button" class="ma-reveal-final" data-reveal-stage="FINAL" ${stage === "TWIST" || stage === "SECOND" ? "" : "disabled"}><span>05</span><strong>Về đích</strong><small>Quán quân băng vạch + pháo hoa</small></button></div><div class="ma-reveal-footer"><p><i></i>Màn đua thuyền tự đồng bộ trong khoảng 1 giây.</p><button type="button" data-reveal-stage="IDLE" ${stage === "IDLE" ? "disabled" : ""}>↻ Đặt lại</button></div></section><aside class="ma-panel ma-reveal-scoreboard"><header><div><small>CHỈ ADMIN</small><h2>Điểm thật</h2></div></header><div>${resultRows}</div></aside></div></section>`;
  }
  function artView() {
