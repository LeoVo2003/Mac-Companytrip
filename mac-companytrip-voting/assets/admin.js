(() => {
  ...
  async function previewImport(file) {
    ...
    modal.querySelector("#ma-import-confirm").addEventListener("click", async (event) => {
      ...
      try {
        ...
        importFeedback = { message: result.message, fileName: file.name, at: new Date().toLocaleTimeString("vi-VN", { hour: "2-digit", minute: "2-digit" }) };
  ...
  const topActions = () => `<div class="ma-top-actions"><button id="ma-refresh">↻ Tải lại dữ liệu</button><button id="ma-reset-event" class="ma-reset-trigger">Đặt lại sự kiện</button></div>`;
  ...
  function sidebar() { return `<aside class="ma-side"><div class="ma-brand"><img src="${esc(window.MACVotingAdmin.logo)}"><strong>Company Trip</strong></div><nav><button data-tab="overview" class="${tab === "overview" ? "active" : ""}">◉ Tổng quan</button><button data-tab="checkin" class="${tab === "checkin" ? "active" : ""}">▣ Check-in</button><button data-tab="art" class="${tab === "art" ? "active" : ""}">♪ Văn nghệ</button><button data-tab="data" class="${tab === "data" ? "active" : ""}">▦ Nhân sự & QR</button></nav><div class="ma-side-links"><a href="${esc(window.MACVotingAdmin.checkinUrl)}" target="_blank" rel="noopener">↗ Máy quét BTC</a><a href="${esc(window.MACVotingAdmin.resultsUrl)}" target="_blank" rel="noopener">↗ Màn hình kết quả</a><a href="${esc(window.MACVotingAdmin.voteUrl)}" target="_blank" rel="noopener">↗ Mở trang vote</a></div></aside>`; }
  function votingGate() {
    const on = !!data.votingEnabled;
    return `<section class="ma-panel ma-gate ${on ? "is-on" : "is-off"}"><header><div><small>VĂN NGHỆ</small><h2>${on ? "Cổng đang bật" : "Cổng đang tắt"}</h2><p>${on ? "QR cá nhân và login username đã được mở." : "Public không thể đăng nhập hoặc chấm điểm, kể cả khi còn cookie cũ."}</p></div><span class="ma-gate-status">${on ? "ĐANG BẬT" : "TẮT"}</span></header><div style="padding:16px 20px 20px"><button type="button" id="ma-gate" class="${on ? "danger" : "ma-primary"}">${on ? "Tắt cổng văn nghệ" : "Bật cổng chấm điểm"}</button></div></section>`;
  }
  const checkpointStatus = (status) => status === "OPEN" ? "Đang mở" : status === "CLOSED" ? "Đã chốt" : "Chưa mở";
  function checkinView() {
    const boards = data.checkinBoard || [];
    const points = data.teamPoints || [];
    const staff = data.staff || [];
    const users = data.assignableUsers || [];
    const teams = data.teams || [];
    return `<header class="ma-top"><div><small>CHECK-IN</small><h1>4 mốc Company Trip</h1></div>${topActions()}</header><div class="ma-check-grid">${(data.checkpoints || []).map((item) => `<article class="ma-check-card ${item.status.toLowerCase()}"><small>MỐC ${item.id} · ${checkpointStatus(item.status)}</small><h2>${esc(item.name)}</h2><p>${esc(item.description || "")}</p><div class="ma-check-actions">${item.status === "DRAFT" ? `<button type="button" class="ma-primary" data-checkpoint="${item.id}" data-op="open">Mở mốc</button>` : item.status === "OPEN" ? `<button type="button" class="danger" data-checkpoint="${item.id}" data-op="close">Đóng & chốt</button>` : `<button type="button" data-checkpoint="${item.id}" data-op="reopen">Mở lại</button>`}</div></article>`).join("")}</div><section class="ma-panel"><header><div><small>TIẾN ĐỘ</small><h2>Team đã đủ / chưa đủ</h2></div><a class="ma-primary" href="${esc(window.MACVotingAdmin.checkinExportUrl)}">↓ Xuất CSV check-in</a></header>${boards.map((board) => `<div class="ma-board-table" style="margin:12px 16px 16px"><table><thead><tr><th>Mốc ${board.checkpoint.id} · ${esc(board.checkpoint.name)}</th><th>Tiến độ</th><th>Hoàn thành</th><th>Hạng</th><th>Điểm</th></tr></thead><tbody>${board.teams.map((team) => `<tr><td><strong>#${team.teamNumber} ${esc(team.teamName)}</strong></td><td>${team.checkedIn}/${team.eligible}</td><td>${team.completedAt || "—"}</td><td>${team.temporaryRank || "—"}</td><td>${team.temporaryPoints || 0}</td></tr>`).join("")}</tbody></table></div>`).join("")}</section><section class="ma-panel"><header><div><small>ĐIỂM CHECK-IN</small><h2>Sổ điểm 4 mốc</h2></div></header><div class="ma-board-table" style="margin:0"><table><thead><tr><th>Team</th><th>Mốc 1</th><th>Mốc 2</th><th>Mốc 3</th><th>Mốc 4</th><th>Tổng</th></tr></thead><tbody>${points.map((row) => `<tr><td><strong>#${row.teamNumber} ${esc(row.teamName)}</strong></td>${row.checkpoints.map((value) => `<td>${value > 0 ? `+${value}` : value}</td>`).join("")}<td><strong>${row.total}</strong></td></tr>`).join("") || `<tr><td colspan="6">Chưa có điểm đã chốt.</td></tr>`}</tbody></table></div></section><section class="ma-panel"><header><div><small>BTC</small><h2>Tài khoản máy quét</h2></div></header><form class="ma-staff-form" id="ma-staff-form"><label>Chọn tài khoản WordPress<select id="ma-staff-user">${users.map((user) => `<option value="${user.id}">${esc(user.name)} · ${esc(user.email)}</option>`).join("")}</select></label><div class="ma-staff-teams">${teams.map((team) => `<label><input type="checkbox" name="teamIds" value="${team.id}"> #${team.team_no} ${esc(team.name)}</label>`).join("")}</div><button type="submit" class="ma-primary">Lưu quyền check-in</button><p style="margin:0;color:#667085;font-size:13px">Admin quét được mọi team. BTC thường chỉ được gán 1-2 team. Máy quét: <a href="${esc(window.MACVotingAdmin.checkinUrl)}" target="_blank" rel="noopener">${esc(window.MACVotingAdmin.checkinUrl)}</a></p></form><div class="ma-board-table" style="margin:0 16px 16px"><table><thead><tr><th>BTC</th><th>Team được gán</th></tr></thead><tbody>${staff.map((item) => `<tr><td><strong>${esc(item.name)}</strong><small>${esc(item.email)}${item.isAdmin ? " · Admin" : ""}</small></td><td>${item.isAdmin ? "Tất cả team" : (item.teamIds || []).map((id) => { const team = teams.find((row) => String(row.id) === String(id)); return team ? `#${team.team_no} ${team.name}` : id; }).join(", ") || "Chưa gán"}</td></tr>`).join("") || `<tr><td colspan="2">Chưa có tài khoản BTC.</td></tr>`}</tbody></table></div>      </section>`;
  }
  function pointsView() {
  ...
  function render() {
    ...
    if (tab === "data") {
      ...
      root.querySelector(".ma-content")?.insertAdjacentHTML("beforeend", teamManager());
    }
    if (tab === "art") {
      let previousScore = null; let currentRank = 0;
