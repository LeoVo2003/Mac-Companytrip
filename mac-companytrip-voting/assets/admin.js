(() => {
  ...
  function revealView() {
    ...
    return `<section class="ma-art-block"><header class="ma-art-block-head"><div><small>CÔNG BỐ VĂN NGHỆ</small><h2>One Direction · The Spotlight</h2></div><a class="ma-screen-link" href="${esc(window.MACVotingAdmin.artResultsUrl)}" target="_blank" rel="noopener">↗ Màn hình trình chiếu</a></header><div class="ma-reveal-grid"><section class="ma-panel ma-reveal-control"><header><div><small>LIVE REVEAL</small><h2>Tín hiệu MC · mỗi lần một đội</h2></div><span class="ma-reveal-status ${stage.toLowerCase()}"><i></i>${revealStageLabel(stage)}</span></header><div class="ma-reveal-intro"><strong>Sáu nhịp spotlight</strong><p>Mỗi nút chỉ mở đúng một đội, đi từ hạng 6 đến quán quân.</p></div><div class="ma-reveal-actions"><button type="button" class="ma-reveal-start" data-reveal-stage="ROLLING" ${stage === "IDLE" ? "" : "disabled"}><span>00</span><strong>Mở màn · The Spotlight</strong><small>Spotlight bắt đầu tìm kiếm</small></button><button type="button" data-reveal-stage="SIXTH" ${stage === "ROLLING" ? "" : "disabled"}><span>01</span><strong>Công bố hạng 6</strong><small>Mở đúng một đội</small></button><button type="button" data-reveal-stage="FIFTH" ${stage === "SIXTH" ? "" : "disabled"}><span>02</span><strong>Công bố hạng 5</strong><small>Mở đúng một đội</small></button><button type="button" data-reveal-stage="FOURTH" ${stage === "FIFTH" ? "" : "disabled"}><span>03</span><strong>Công bố hạng 4</strong><small>Mở đúng một đội</small></button><button type="button" data-reveal-stage="THIRD" ${stage === "FOURTH" ? "" : "disabled"}><span>04</span><strong>Công bố hạng 3</strong><small>Mở đúng một đội</small></button><button type="button" data-reveal-stage="SECOND" ${stage === "THIRD" ? "" : "disabled"}><span>05</span><strong>Công bố hạng 2</strong><small>Mở đúng một đội</small></button><button type="button" class="ma-reveal-final" data-reveal-stage="FINAL" ${stage === "SECOND" ? "" : "disabled"}><span>06</span><strong>Công bố quán quân</strong><small>Khóa spotlight · bùng nổ</small></button></div><div class="ma-reveal-footer"><p><i></i>Màn hình trình chiếu tự đồng bộ trong khoảng 1 giây.</p><div class="ma-reveal-footer-actions"><button type="button" id="ma-art-theme" class="${lightTheme ? "is-warn" : ""}">${lightTheme ? "◎ Về tone tối" : "◎ Bật tone đại dương"}</button><button type="button" data-reveal-stage="IDLE" ${stage === "IDLE" ? "disabled" : ""}>↻ Đặt lại</button></div></div></section><aside class="ma-panel ma-reveal-scoreboard"><header><div><small>CHỈ ADMIN</small><h2>Điểm thật</h2></div></header><div>${resultRows}</div></aside></div></section>`;
  }
  function artView() {
