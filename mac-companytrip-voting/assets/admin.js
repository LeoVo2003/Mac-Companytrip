(() => {
  ...
  async function previewImport(file) {
    ...
    const validationMessage = serverError
      ...
      : `<div class="ma-import-ready"><strong>✓ ${totalRows} người hợp lệ</strong><span>${esc(serverValidation?.message || "Dữ liệu đã được kiểm tra.")}</span><small>${Number(serverValidation?.companions || 0)} người đi kèm · ${Number(serverValidation?.familyGroups || 0)} nhóm người thân · ${Number(serverValidation?.roomGroups || 0)} nhóm chung phòng</small></div>`;
