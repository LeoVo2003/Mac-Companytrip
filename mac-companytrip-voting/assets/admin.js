(() => {
  ...
  const refreshCategoryMeta = (categoryId, teams) => {
    ...
    category.isComplete = values.length === teams.length;
  };
  ...
  function revealView() {
    ...
    return `<section class="ma-art-block"><header class="ma-art-block-head"><div><small>CÔNG BỐ</small><h2>Đua thuyền văn nghệ</h2></div><a class="ma-screen-link" href="${esc(window.MACVotingAdmin.artResultsUrl)}" target="_blank" rel="noopener">↗ Màn hình trình chiếu</a></header><div class="ma-reveal-grid"><section class="ma-panel ma-reveal-control"><header><div><small>LIVE REVEAL</small><h2>Tín hiệu MC</h2></div><span class="ma-reveal-status ${stage.toLowerCase()}"><i></i>${revealStageLabel(stage)}</span></header><div class="ma-reveal-intro"><strong>Ba lần chốt giải</strong><p>Hạng 3 → hạng 2 → quán quân, thuyền về bến theo thứ hạng.</p></div><div class="ma-reveal-actions"><button type="button" class="ma-reveal-start" data-reveal-stage="ROLLING" ${stage === "IDLE" ? "" : "disabled"}><span>00</span><strong>Mở màn · Đua thuyền</strong><small>6 thuyền ra khơi</small></button><button type="button" data-reveal-stage="THIRD" ${stage === "ROLLING" || stage === "DECOY" ? "" : "disabled"}><span>01</span><strong>Công bố hạng 3</strong><small>Thuyền hạng ba về bến</small></button><button type="button" data-reveal-stage="SECOND" ${stage === "THIRD" ? "" : "disabled"}><span>02</span><strong>Công bố hạng 2</strong><small>Thuyền hạng nhì về bến</small></button><button type="button" class="ma-reveal-final" data-reveal-stage="FINAL" ${stage === "SECOND" ? "" : "disabled"}><span>03</span><strong>Công bố quán quân</strong><small>Quán quân chạm bến</small></button></div><div class="ma-reveal-footer"><p><i></i>Màn hình trình chiếu tự đồng bộ trong khoảng 1 giây.</p><button type="button" data-reveal-stage="IDLE" ${stage === "IDLE" ? "disabled" : ""}>↻ Đặt lại</button></div></section><aside class="ma-panel ma-reveal-scoreboard"><header><div><small>CHỈ ADMIN</small><h2>Điểm thật</h2></div></header><div>${resultRows}</div></aside></div></section>`;
  }
  function artView() {
