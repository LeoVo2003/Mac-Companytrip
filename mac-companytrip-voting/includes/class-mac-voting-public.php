...
final class MAC_Voting_Public {
    ...
    public static function register_assets(): void {
        ...
        wp_register_style('mac-voting-art-race', MAC_VOTING_URL . 'assets/art-race.css', array(), MAC_VOTING_VERSION);
        wp_register_script('mac-voting-art-race', MAC_VOTING_URL . 'assets/art-race.js', array(), MAC_VOTING_VERSION, true);
        ...
        if (self::is_art_race_page()) {
            wp_enqueue_style('mac-voting-art-race');
            wp_enqueue_script('mac-voting-art-race');
        }
    }
    ...
    private static function is_results_page(): bool {
        ...
        return ($page_id && is_page($page_id))
            || ($post instanceof WP_Post && has_shortcode($post->post_content, 'mac_companytrip_results'));
    }

    private static function is_art_race_page(): bool {
        global $post;
        $page_id = (int) get_option('mac_voting_art_results_page_id');
        return ($page_id && is_page($page_id))
            || ($post instanceof WP_Post && has_shortcode($post->post_content, 'mac_companytrip_art_race'));
    }
    ...
    public static function art_race_shortcode(): string {
        wp_enqueue_style('mac-voting-art-race');
        wp_enqueue_script('mac-voting-art-race');
